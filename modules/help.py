    ...
    async def help(c: Client, m: types.Message = None):
        ...
        if len(words) == 1:
            await m.edit(res_header(c.name, m.text) + 'Список команд:\n' +
                         ...
                         '<code>+farm [rat] [pog]</code> - включить или выключить автофарм\n'
            ...
            match words[1]:
                case 'farm':
                    body = '<code>+farm [rat] [pog]</code> - включить или выключить автопатруль с возможностью автоматического создания мешков с золотом.\n' +\
                        'Если указано rat, то вместо автопатруля будет включена автоматическая охота на крыс.\n' +\
